...
SearchManager.prototype.organizeSearchOptions = function(options){
  ...
  if(options.filter === 'people'){
    ...
    this.app.elasticsearch.addTermFacet('fee', obj);
  }
...
SearchManager.prototype.runSearch = function(obj, options, callback){
  ...
  console.log(params);
  this.app.elasticsearch.search(obj, options.filer, params, function(err, results){
    if(err) callback(err, null);
      ...
      if(results.hits){
        ...
        for(var i in results.hits.hits){
          ...
          result.person = (results.hits.hits[i]._type === 'people');
          result.article = (results.hits.hits[i]._type === 'articles');
      ...
      var page = options.fromIndex / options.resultsPerPage;
      page = (page === 0 ? 1 : page);
      var data = {results: resultList, total: total, facets: facetCounts, page: page};
      callback(null, data);
    }
  });
};
