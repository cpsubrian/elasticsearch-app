...
SearchManager.prototype.organizeSearchOptions = function(options){
  ...
  if(options.filter === 'people'){
    ...
    queryBuilder.addTermFacet('region');
    queryBuilder.addTermFacet('issue');
  }else if (options.filter === 'articles'){
    queryBuilder.addTermFacet('section');
    queryBuilder.addTermFacet('tag');
  }

  //If 'all', just send back null type
  if(options.filter === 'all')
    options.filter = 'articles,people';
